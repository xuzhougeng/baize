...
- There is only one legal entry point for automatic tool use: the agent loop chooses tools from `ListAgentToolDefinitions` and executes them through `ExecuteAgentTool`. Do not add side paths that detect a tool need in message dispatch and then call tool-specific logic directly.
- Pre-filtering which tools are exposed to the agent loop is allowed when it stays declarative and upstream of selection, for example interface-based availability, user-controlled enable/disable switches, or future capability narrowing. That filtering must only change the candidate set; it must not bypass the agent loop or execute tool logic outside it.
- Desktop and WeChat must expose the same agent tool candidate set by default. Do not hide a tool from WeChat just because the caller is WeChat; only platform impossibility or an explicit user-controlled disable switch should remove a tool from `ListAgentToolDefinitions`.
...
- A thinner interface may expose fewer presentation affordances, but it must not redefine the fundamental conversation semantics or silently shrink the desktop/WeChat agent tool set.
...
- Desktop-only background features such as `ScreenTrace` must stay adapter-thin: lifecycle, settings persistence, and presentation can live in desktop code, but capture analysis, deduplication, digest generation, and storage should live in `internal/<domain>` packages.
