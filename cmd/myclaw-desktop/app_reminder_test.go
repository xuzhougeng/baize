...
func TestListRemindersFiltersDesktopPrimaryAndFormatsOutput(t *testing.T) {
	t.Parallel()

	ctx := context.Background()
	root := t.TempDir()
	store := reminder.NewStore(filepath.Join(root, "reminders.json"))
	...
	if _, err := store.Add(ctx, reminder.Reminder{
		Target:    reminder.Target{Interface: desktopInterface, UserID: "secondary"},
		Message:   "不应出现在 primary 列表",
		Frequency: reminder.FrequencyOnce,
		NextRunAt: time.Date(2026, 3, 29, 9, 0, 0, 0, time.Local),
		CreatedAt: createdAt,
	}); err != nil {
		t.Fatalf("seed other target reminder: %v", err)
	}
	...
	if len(items) != 2 {
		t.Fatalf("expected 2 reminders, got %d", len(items))
	}

	if items[0].Message != "交房租" {
	...
	if items[0].Frequency != string(reminder.FrequencyOnce) || items[0].FrequencyLabel != "单次" || items[0].ScheduleLabel != "单次" {
		t.Fatalf("unexpected once reminder formatting: %#v", items[0])
	}
	if items[0].NextRunAt != "2026-03-30 14:00:00" {
		t.Fatalf("unexpected once next run: %#v", items[0])
	}

	if items[1].Message != "写日报" {
		t.Fatalf("unexpected second reminder: %#v", items[1])
	}
	if items[1].Frequency != string(reminder.FrequencyDaily) || items[1].FrequencyLabel != "每天" || items[1].ScheduleLabel != "每天 09:30" {
		t.Fatalf("unexpected daily reminder formatting: %#v", items[1])
	}
	if items[1].CreatedAt != "2026-03-28 09:10:00" {
		t.Fatalf("unexpected daily created at: %#v", items[1])
	}
}
