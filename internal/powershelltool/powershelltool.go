...
func UsageText() string {
	...
	if len(names) == 0 {
		return "PowerShell Tool is only enabled on Windows."
	}
	return strings.TrimSpace(fmt.Sprintf(`
Tool: %s
Purpose: inspect a Windows machine with one allowlisted read-only PowerShell command.

Input:
- command: required allowlisted PowerShell command name
...
Use this when the user needs current host/process/network/basic PowerShell-visible state on Windows.
...
func Execute(ctx context.Context, input ToolInput) (ToolResult, error) {
	...
	stdout, stderr, exitCode, err := runCommand(execCtx, powerShellProgram(), "-NoProfile", "-Command", commandLine)
	result := ToolResult{
		Tool:     ToolName,
		Shell:    "powershell",
		Command:  spec.Name,
	...
	default:
		...
		return result, fmt.Errorf("run powershell command %q: %w", spec.Name, err)
	}
}
...
func commandCatalog(goos string) map[string]commandSpec {
	...
	return map[string]commandSpec{
		"get-computerinfo": {Name: "Get-ComputerInfo", ArgVariants: [][]string{{}}},
		"get-date":         {Name: "Get-Date", ArgVariants: [][]string{{}}},
		"get-location":     {Name: "Get-Location", ArgVariants: [][]string{{}}},
		"get-netipaddress": {Name: "Get-NetIPAddress", ArgVariants: [][]string{{}}},
		"get-process":      {Name: "Get-Process", ArgVariants: [][]string{{}}},
		"get-psdrive":      {Name: "Get-PSDrive", ArgVariants: [][]string{{}}},
